    ...
    internal class CoolantController : ModelController
    {
        ...
        public void CreateCoolantHoles()
        {
            ...
            if (Properties.ToolType != "Blank")
            {
                ...
                coolantExitFluteRotation = (Properties.LOC - Properties.LOA + Properties.CoolantHoleLength) / (decimal)GeneratedProperties.HelixPitch * 360 + 45;
            }
            EquationController.SetEquation("CoolantExitFluteRotation", $"{coolantExitFluteRotation}");

            EquationController.SetEquation("CoolantStartLength", $"{Properties.CoolantHoleLength}in");
            EquationController.SetEquation("CoolantExitAngle", $"{Properties.CoolantHoleAngle}");
            EquationController.SetEquation("CoolantHoleDiameter", $"{Properties.CoolantHoleDiameter}in");
            EquationController.SetEquation("CoolantFeedHoleDiameter", $"{Properties.CoolantFeedDiameter}in");
            EquationController.SetEquation("CoolantCount", $"{Properties.CoolantHoleCount}");
            EquationController.SetEquation("CoolantSpacing", $"{Properties.CoolantHoleRotation}");
            ...
            decimal rotationAngle = Properties.CoolantPatternAngle;

            if (Properties.CoolantPatternAlongFluting)
            {
                ...
                rotationAngle += 360m * Properties.CoolantPatternLength / (decimal)GeneratedProperties.HelixPitch;
            }

            if (Properties.CoolantPatternCount != 1)
            {
                double coolantPatternSpacing = Math.Sqrt(Math.Pow(decimal.ToDouble(Properties.CoolantPatternLength), 2) + Math.Pow(decimal.ToDouble(rotationAngle) * Math.PI * decimal.ToDouble(Properties.ToolDiameter) / 360, 2));
                double coolantPatternPitch = 360 * decimal.ToDouble(Properties.CoolantPatternLength) / decimal.ToDouble(rotationAngle);
            ...
            EquationController.SetEquation("CoolantHelixCount", $"{Properties.CoolantPatternCount}");
